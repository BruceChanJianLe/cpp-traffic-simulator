    ...
    while (true)
    {
        ...
        if (timeSinceLastUpdate >= cycleDuration)
        {
            ...
            if (completion >= 0.9 && !hasEnteredIntersection)
            {
                ...
                // Wait for response (wait for addVehicleToQueue to finish)
                ftrEntryGranted.get();
            ...
            if (completion >= 1.0 && hasEnteredIntersection)
            {
                ...
                if (streetOptions.size() > 0)
                {
                    ...
                    // random_device will generate a random unsigned int
                    std::random_device rd;
                    // mt19937 is one of the random engines provided by C++, we seed it with the value from rd()
                    std::mt19937 eng(rd());
                    // uniform_int_distribution make sure that the value is between min and max when given a value
