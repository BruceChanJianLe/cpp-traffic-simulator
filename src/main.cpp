...
{
    // Assign filename to corresponding city map
    ...
    for(size_t ni = 0; ni < nIntersections; ni++)
    {
        intersections.emplace_back(std::make_shared<Intersection>());
    }
    // Position intersection (in pixel coordinate)
    ...
    for(size_t ns = 0; ns < nStreets; ns++)
    {
        streets.emplace_back(std::make_shared<Street>());
    ...
    for(size_t nv = 0; nv < nVehicles; nv++)
    {
        vehicles.emplace_back(std::make_shared<Vehicle>());
        vehicles.at(nv)->setCurrentStreet(streets.at(nv));
